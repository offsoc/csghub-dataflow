...
from data_server.utils.jwt_utils import parse_jwt_token, JWTDecodeError
...
async def get_algo_templates(
    ...
    buildin: bool = Query(description="是否为内置模版过滤"),
    ...
    Args:
        authorization: Authorization header，格式: "Bearer <jwt_token>"
        name: 模板名称（模糊查询）
    ...
    try:
        ...
        if token_info["is_expired"]:
            return response_fail(msg="Token已过期，请重新登录")
        
        # 获取用户user_id
        user_id = token_info["payload"].get("uuid")
        if not user_id:
            return response_fail(msg="Token中缺少用户信息")
        ...
        return response_fail(msg="Token无效")
